...
E_local = [];
for i = 1: size(g1.faces, 2)
    all_neighbor = g1.faces{i}.all_neighbors;
    for j = 1: size(all_neighbor, 1)
        if g.faces{all_neighbor(j)}.state == 2
            E_local = [E_local; g1.faces{i}.id all_neighbor(j)];
        end
    end
end
...
for i = 1: size(g.faces, 2)
    if g.faces{i}.state == 2
        all_neighbor = g.faces{i}.all_neighbors;
        for j = 1: size(all_neighbor, 1)
            if g.faces{all_neighbor(j)}.state == 2
                E_local = [E_local; g.faces{i}.id all_neighbor(j)];
            end
        end
    end
    all_neighbor = g.faces{i}.all_neighbors;
    for j = 1: size(all_neighbor)
        E_global = [E_global; g.faces{i}.id all_neighbor(j)];
    end
end
