...
%     end
    ...
    for f = 1 : length(neighbors)
        ...
        if g.faces{adj_id}.state == 0
            ...
            global_measure = dot(-adj_normal, global_center_diff); % Attempt 2: projection 
            % (global_center_diff is normalized, so |b|*cos(theta) = projection)
            if s < 11404/10
                test_t = -0.5;
            else
                test_t = 0.3;
...
%     faces_new = [faces_new new_g.faces{i}.nodes'];
%     faces_id = [faces_id new_g.faces{i}.id];
% end
% 
% vertex_new = [];
% vertex_id = [];
% for i = 1:size(new_g.nodes, 2)
%     vertex_new = [vertex_new new_g.nodes{i}.coor'];
%     vertex_id = [vertex_id new_g.nodes{i}.id];
% end

% g1 = Initialize_G_w_id(vertex_new, vertex_id, faces_new, faces_id, 2);
% E_local = [];
% for i = 1: size(new_g.faces, 2)
%     all_neighbor = new_g.faces{i}.all_neighbors;
%     for j = 1: size(all_neighbor, 1)
%         if g.faces{all_neighbor(j)}.state == 2
%             E_local = [E_local; new_g.faces{i}.id all_neighbor(j)];
%         end
%     end
% end

% Get the corresponded E and V matrix

E_local = [];
E_global = [];
for i = 1: size(g.faces, 2)
    if g.faces{i}.state == 2
        for j = 1: size(g.faces{i}.all_neighbors, 1)
            if g.faces{g.faces{i}.all_neighbors(j)}.state == 2
                E_local = [E_local; g.faces{i}.id g.faces{i}.all_neighbors(j)];
            end
        end
    end
    for j = 1: size(g.faces{i}.all_neighbors)
        E_global = [E_global; g.faces{i}.id g.faces{i}.all_neighbors(j)];
    end
end
...
[count, g_new] = optimize_path(E_local, E_global, faces', boundary_points, 0.001, g); 
% E_visited = [];
% for i = 1:size(new_g.faces, 2)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
new_g1 = Get_small(g_new, 2); % 0:not-visited 1:visited 2:boundary
new_g2 = Get_small(g, 2);
v = Extract_v(new_g2); % Get all the vertex
f = Extract_f(new_g2); % Get all the faces
plot_mesh(v, f, options); % Start plotting
options.face_vertex_color = 0.3*ones(size(v, 2), 1);
clf;
plot_mesh(v, f, options);
